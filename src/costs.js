...
((root,factory)=>{const api=factory();if(typeof module==='object')module.exports=api;else root.costs=api;})(globalThis,()=>{
  ...
  function quote(model,input,tariffs){
    ...
    if(model.apiModel==='grok-imagine-video-1-5-preview'){
    ...
    const rows=(tariffs?.rows||[]).filter(row=>row.modelDescription?.toLowerCase()===description.toLowerCase());
    if(rows.length!==1)return null;
    const row=rows[0],unit=amount(row.creditPrice);
    if(unit===null||row.creditUnit!==expectedUnit)return null;
    return {credits:round(unit*multiplier),description,tariffDate:tariffs.fetchedAt,source:'https://kie.ai/pricing',type:'tariff'};
  }
  function charge(record,currentRate){
    const credits=amount(record.creditsConsumed);
    const stored=amount(record.rubPerCredit);
    const rate=stored??currentRate;
    return {credits,rubles:credits===null?null:round(credits*rate),rate,historical:stored!==null};
  }
  ...
  return {amount,round,quote,charge,summary,providerSnapshot,reconcile};
});
