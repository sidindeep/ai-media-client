...
app.whenReady().then(async () => {
  ...
  try {
    ...
    const mediaGeometry = await evaluate(`(async()=>{
      const card=document.querySelector('.chat-result-card');
      const media=document.createElement('div');media.className='chat-result-media';
      const image=document.createElement('img');const video=document.createElement('video');
      media.append(image,video);card.append(media);
      const before={image:image.getBoundingClientRect().height,video:video.getBoundingClientRect().height};
      await new Promise((resolve,reject)=>{image.onload=resolve;image.onerror=reject;image.src='data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+jRZkAAAAASUVORK5CYII='});
      const after={image:image.getBoundingClientRect().height,video:video.getBoundingClientRect().height};
      media.remove();return {before,after};
    })()`);
    assert.ok(mediaGeometry.before.image > 100 && mediaGeometry.before.video > 100, 'chat previews reserve space before media metadata arrives');
    assert.ok(Math.abs(mediaGeometry.before.image - mediaGeometry.after.image) < 1 && Math.abs(mediaGeometry.before.video - mediaGeometry.after.video) < 1, 'loaded media does not resize the chat feed');
    await evaluate("Array.from(document.querySelectorAll('.sidebar-list .list-item')).find(button=>button.textContent.includes('Без чата')).click();void 0");
    await until("document.querySelector('.studio-header').textContent.includes('Без чата') && document.querySelector('.chat-results').scrollHeight-document.querySelector('.chat-results').clientHeight-document.querySelector('.chat-results').scrollTop<=32");
    assert.equal(await evaluate("document.querySelectorAll('.chat-result-item').length"), 1, 'the system feed contains only generations without a chat');
    assert.match(await evaluate("document.querySelector('.chat-result-item').textContent"), /Запрос обычного пользователя/);
    assert.equal(await evaluate(`document.querySelector('.chat-result-item[data-record-id=${JSON.stringify(newestCodexId)}]')`), null, 'another chat generation must not leak into the system feed');
    assert.match(await evaluate("Array.from(document.querySelectorAll('.sidebar-list .list-item')).find(button=>button.textContent.includes('Без чата')).textContent"), /1 генерация/);
    await evaluate("Array.from(document.querySelectorAll('.sidebar-tabs button')).find(button=>button.textContent.trim()==='Проекты').click();void 0");
    await until("document.querySelector('.project-list-view')?.textContent.includes('Vue проект')");
    await evaluate("Array.from(document.querySelectorAll('.project-list-view .list-item')).find(button=>button.textContent.includes('Vue проект')).click();void 0");
    await until("document.querySelector('.project-children')?.textContent.includes('Vue чат')");
    ...
    assert.ok(await evaluate(`document.querySelector('.chat-result-item[data-record-id=${JSON.stringify(newestCodexId)}]')`), 'returning to the chat restores its own generations');
    assert.equal(await evaluate("document.querySelector('.chat-results').textContent.includes('Запрос обычного пользователя')"), false, 'unassigned generations must not leak into a named chat');
    ...
    const focusBeforeReload = await evaluate(`(async()=>{
      const feed=document.querySelector('.chat-results');
      feed.scrollTop=90;feed.dispatchEvent(new Event('scroll'));
      await new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));
      const saved=JSON.parse(localStorage.getItem(${JSON.stringify(chatScrollStorageKey)}))[${JSON.stringify(vueChat.id)}];
      return {top:feed.scrollTop,saved};
    })()`);
    assert.ok(focusBeforeReload.top >= 60 && focusBeforeReload.saved.anchorId, 'a concrete message and offset are stored before reload');
    assert.ok(Math.abs(focusBeforeReload.saved.top - focusBeforeReload.top) < 2);
    await win.reload();
    await until(`document.querySelector('.studio-header')?.textContent.includes('Vue чат') && document.querySelector('.chat-results')?.clientHeight>0 && Math.abs(document.querySelector('.chat-results').scrollTop-${focusBeforeReload.top})<3`);
    assert.equal(await evaluate(`JSON.parse(localStorage.getItem(${JSON.stringify(chatScrollStorageKey)}))[${JSON.stringify(vueChat.id)}].anchorId`), focusBeforeReload.saved.anchorId, 'reload preserves the focused message');
