...
const timeline = computed(() => {
  ...
  return [
    step('Принято приложением', 'Запрос сохранён и готов к запуску.', 'done', queuedAt),
    step('Ожидание свободного слота', record.state === 'queued' ? 'Ожидаем доступный параллельный слот.' : 'Свободный слот получен.', record.state === 'queued' ? 'active' : record.preparingAt ? 'done' : 'pending', queuedAt, stepDuration(queuedAt, record.preparingAt, record.state === 'queued')),
