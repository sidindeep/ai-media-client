...
function createTelegramBot({ service, directory, allowedUsers = [], publicAccess = true, downloadFile, resolveAccount, completeLink }) {
  ...
  async function session(id, allModels) {
  ...
  async function handle(update) {
    ...
    const telegramUser = update.callback_query?.from || message.from;
    const telegramUserId = String(telegramUser?.id || chatId);
    const text = (update.callback_query?.data || message.text || '').trim();
    const start = /^\/(start)(?:@\w+)?(?:\s+(\S+))?$/.exec(text);
    if (start?.[2] && completeLink) {
      await completeLink(start[2], telegramUserId, telegramUser?.username);
      return reply('Telegram привязан к вашему аккаунту. Теперь здесь доступны ваши задачи и баланс аккаунта.', menu);
    }
    const context = resolveAccount ? await resolveAccount(telegramUserId) : { service };
    ...
    if (text.startsWith('/set ')) {
      ...
      await save({ input: { ...data.input, [match[1]]: value }, confirmation: null }); return draftReply(data, allModels, activeService);
    }
    if (text.startsWith('/params ')) {
      ...
      await save({ input, confirmation: null }); return draftReply(data, allModels, activeService);
    }
    if (text === '/new') { await save({ input: defaults(model), sourceFiles: [], confirmation: null }); return draftReply(data, allModels, activeService); }
    if (text === '/clear') {
      const input = { ...data.input }; for (const field of model.fields.filter(item => item.type === 'files')) delete input[field.key];
      await save({ input, sourceFiles: [], confirmation: null }); return draftReply(data, allModels, activeService);
    }
    if (text === 'generate' || text === '/generate') {
      if (!activeService.configured()) return reply('Генерация ещё не подключена на сервере. Черновик сохранён.', menu);
      activeService.validate(model, data.input);
      let quote = null;
    ...
    if (text.startsWith('go:')) {
      ...
      const requestId = `telegram:${chatId}:${token}`;
      const task = context.accountId
        ? await activeService.createTelegramTask(data.confirmation.request, token)
        : activeService.dispatch ? await activeService.dispatch('createTask', [{ ...data.confirmation.request, requestId }]) : await activeService.createTask({ ...data.confirmation.request, requestId });
      if (!context.accountId) await activeService.history.update(task.id, { telegramChatId: chatId });
      if (activeService.dispatch) await activeService.dispatch('startQueue'); else activeService.queue.start();
      return reply(`Задача добавлена: ${task.modelName || task.modelId || model.name}\n${task.id}\nО готовности сообщу здесь.`, menu);
    }
    const operator = allowed.has(chatId);
    const taskPrefix = `telegram:${context.accountId ? `${context.accountId}:` : ''}`;
    const visibleTasks = async () => (await activeService.listHistory()).filter(item => context.accountId
      ? item.requestId?.startsWith(taskPrefix)
      : operator || item.telegramChatId === chatId);
    if (['queue', '/queue'].includes(text)) {
      ...
      const rows = !context.accountId && operator ? [[button(activeService.queue.paused ? 'Продолжить' : 'Пауза', activeService.queue.paused ? 'resume' : 'pause')]] : [];
      ...
      return reply(`${activeService.queue.paused ? 'Очередь на паузе' : 'Очередь работает'}\n${tasks.slice(0, 10).map(item => `${labels[item.state] || item.state} · ${item.modelName}`).join('\n') || 'Задач нет'}`, rows);
    }
    if (text === 'pause' || text === 'resume') { if (!operator || context.accountId) return reply('Управление общей очередью доступно только оператору.', menu); if (text === 'pause') activeService.queue.pause(); else activeService.queue.start(); return reply('Состояние очереди обновлено.', menu); }
    if (text.startsWith('cancel:')) { const id = text.slice(7); if (!(await visibleTasks()).some(item => item.id === id)) return reply('Задача недоступна.', menu); await activeService.queue.cancel(id); return reply('Ожидающая задача отменена.', menu); }
    if (['history', '/history'].includes(text)) {
      const tasks = (await visibleTasks()).slice(0, 8);
      return reply(tasks.map(item => `${labels[item.state] || item.state} · ${item.modelName}\n${activeService.resultUrls(item).join('\n')}`).join('\n\n') || 'История пока пуста.', menu);
    }
    ...
    if (attachment) {
      ...
      const source = await activeService.saveSource({ name: attachment.file_name || `source.${kind.split('/')[1]}`, type: kind, bytes });
    ...
    if (text && !update.callback_query) { await save({ input: { ...data.input, prompt: text }, confirmation: null }); return draftReply(data, allModels, activeService); }
