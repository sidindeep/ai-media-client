...
function options(input = {}) {
  ...
  const custom = input.from != null || input.to != null;
  const days = custom ? null : Number(input.days ?? 30);
  const category = String(input.category ?? 'all');
  if ((!custom && !PERIODS.has(days)) || !CATEGORIES.has(category)) throw Object.assign(new Error('Некорректный фильтр расходов'), { status: 400 });
  const from = custom ? new Date(input.from) : null;
  const to = custom ? new Date(input.to) : null;
  if (custom && (!input.from || !input.to || !Number.isFinite(from.getTime()) || !Number.isFinite(to.getTime()) || from >= to || from.getTime() > Date.now()))
    throw Object.assign(new Error('Некорректный диапазон расходов'), { status: 400 });
  const asOf = input.asOf ? new Date(input.asOf) : new Date(custom ? Math.min(to.getTime() - 1, Date.now()) : Date.now());
  if (!Number.isFinite(asOf.getTime()) || asOf.getTime() > Date.now() + 60000 || (custom && (asOf < from || asOf >= to))) throw Object.assign(new Error('Некорректная дата расходов'), { status: 400 });
  let cursor = null;
  ...
  return { days, category, asOf: asOf.toISOString(), since: custom ? from.toISOString() : new Date(asOf.getTime() - days * 86400000).toISOString(), cursor };
}
