...
function normalizeTask(model, data) {
  ...
  if(!model.adapter) return {...data,...providerTiming,kind:model.kind};
  let state;
  ...
  return {taskId:data.taskId,state,kind:model.kind,resultJson:JSON.stringify({resultUrls:urls}),failMsg:data.errorMessage||data.failMsg,creditsConsumed:data.creditsConsumed,progress:model.adapter==='4o' && data.progress!=null ? Math.round(Number(data.progress)*100) : data.progress,...providerTiming};
}
