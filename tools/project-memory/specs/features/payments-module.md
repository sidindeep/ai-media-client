...
- DEBUG-адаптер `yookassa-stub` в test-среде позволяет включить карточки
  ...
  При отсутствии shopId/secretKey в test-среде выбор `yookassa` переходит на
  заглушку. В live-среде отсутствие ключей отключает платежи, но не сайт и
  аккаунты. Ошибка настройки платежей не должна запускать повторное открытие БД.
...
До подключения платёжной системы выбор пакета открывает экран способов оплаты
с выбранной ценой и количеством кредитов. Все способы на этом экране заблокированы
и обозначены как недоступные; выбор пакета сам по себе не создаёт заказ или
платёжную попытку. Список на экране является предварительным показом интерфейса,
а не заявлением о поддержке методов провайдером. После подключения оплаты
доступные способы должны определяться возможностями активного адаптера.
